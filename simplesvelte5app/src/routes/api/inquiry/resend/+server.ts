...
// helper to load env like the send-email endpoint does
function loadEnv() {
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  const row = await get('SELECT Id, Email, Message, Reply FROM Inquiries WHERE Id = ?', [id]);
    ...
    // ensure SendAttempts table exists
    try { await run('CREATE TABLE IF NOT EXISTS SendAttempts (Id INTEGER PRIMARY KEY AUTOINCREMENT, InquiryId INTEGER NOT NULL, AttemptedAt INTEGER NOT NULL, Success INTEGER NOT NULL, MessageId TEXT, Response TEXT)'); } catch (e) {}
    const thirty = Math.floor(Date.now() / 1000) - 30;
    ...
    // persist messageId and mark seen
  try { await run('ALTER TABLE Inquiries ADD COLUMN SentMessageId TEXT'); } catch (e) {}
  await run('UPDATE Inquiries SET SeenByUser = 1, SentMessageId = ? WHERE Id = ?', [info.messageId || null, id]);

  // log attempt
  await run('INSERT INTO SendAttempts (InquiryId, AttemptedAt, Success, MessageId, Response) VALUES (?,?,?,?,?)', [id, Math.floor(Date.now()/1000), success, info.messageId || null, responseText]);
