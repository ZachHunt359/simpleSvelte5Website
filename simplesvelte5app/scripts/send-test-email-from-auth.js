...
console.log('Sending test email FROM authenticated user:', smtpUser, 'TO', to);
...
async function run() {
  try {
    const info = await transporter.sendMail({
      from: smtpUser,
      to,
      subject: `Deliverability test ${Date.now()}`,
      text: `Deliverability test sent at ${new Date().toISOString()} to ${to}`
    });
    console.log('\n== sendMail result ==');
