...
if (process.env.SMTP_USER && process.env.SMTP_PASS) {
  ...
  // No SMTP credentials provided — use a safe JSON transport for testing
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    try {
      ...
      // If the DB check fails, log and continue — we don't want a DB issue to block sending
    ...
    try {
      ...
      // ensure SentMessageId column exists (safe to run multiple times)
    ...
    // Return send info so the UI can show messageId for tracing
