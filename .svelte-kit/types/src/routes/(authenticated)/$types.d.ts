...
type LayoutRouteId = RouteId | "/(authenticated)/admin/invite" | "/(authenticated)/admin/logs" | "/(authenticated)/admin/migrations" | "/(authenticated)/archive" | "/(authenticated)/dashboard" | "/(authenticated)/settings" | "/(authenticated)/upload"
...
export type LayoutServerData = Expand<OptionalUnion<EnsureDefined<Kit.LoadProperties<Awaited<ReturnType<typeof import('../../../../../src/routes/(authenticated)/+layout.server.js').load>>>>>>;
