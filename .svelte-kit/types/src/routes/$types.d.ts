...
type LayoutRouteId = RouteId | "/" | "/(authenticated)/admin/invite" | "/(authenticated)/admin/logs" | "/(authenticated)/admin/migrations" | "/(authenticated)/archive" | "/(authenticated)/dashboard" | "/(authenticated)/settings" | "/(authenticated)/upload" | "/login" | "/logout" | "/register" | "/signup" | "/[chapter]/[panel]" | null
type LayoutParams = RouteParams & { chapter?: string; panel?: string }
